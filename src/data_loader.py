...
from util import compute_roi, resample_image
...
class Loader(object):
    ...
    def get_record(self, patient_key: str, test: bool = False) -> Dict[str, np.ndarray]:
        ...
        t2w_roi = th.from_numpy(t2w_roi).unsqueeze(0)
        anno_roi = th.from_numpy(anno_roi).type(th.int64)
    ...
    def _stack_samples(
        ...
        result_leaves = map(th.stack, grouped_leaves)

...
def test_batch_assembly() -> None:
    ...
    # Expand channel dimension for annotation.
    assert record["images"].shape == record["annotation"].unsqueeze(0).shape

    batch = loader.get_batch(8)
    # Expand channel dimension for annotation.
    assert batch["images"].shape == batch["annotation"].unsqueeze(1).shape
